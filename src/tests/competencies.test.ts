...
// The ids come from the grid's order, not from anything the grid writes.
const C1_ITEM = "C1 — Reading a codebase";
const C2_ITEM = "C2 — Changing it safely";
...
test("a grid declaring two Competencies yields two grade items, two sheets per Student, and two imported columns", async () => {
  ...
  for (const student of [AMINA, BRUNO]) {
    assert.match(
      gradebookRow(workspace, C1_ITEM, student.email)?.sheet ?? "",
      /Can they name the seam before the file\?/
    );
    assert.match(
      gradebookRow(workspace, C2_ITEM, student.email)?.sheet ?? "",
      /One command, run live, that goes red\?/
    );
    assert.equal(gradebookRow(workspace, C1_ITEM, student.email)?.band, "");
  }
});
...
probes:
  ...
  assert.equal(result.code, 1);
  assert.match(result.stderr, /defines no probes for C2/);
  assert.equal(probeSheetsText(workspace), undefined);
});
...
probes:
  ...
  assert.equal(result.code, 1);
  assert.match(result.stderr, /Competency C1/);
