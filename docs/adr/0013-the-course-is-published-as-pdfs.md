...
- **A changed PDF is replaced in its own file resource.** When the hash of a document's markdown and pictures differs from its Manifest entry, or the title it is published under differs from the one the entry records, the run swaps the file, sets the resource's name to the current title, and keeps the module id, the place in the Section and the visibility. A retitle is not in the hash, but the title is printed at the top of the PDF, so there is no rename apart from a replace (#36). An unchanged document uploads nothing.
