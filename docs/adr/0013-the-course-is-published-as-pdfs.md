...
- **A changed PDF is replaced in its own file resource.** When the hash of a document's markdown and pictures differs from its Manifest entry, the run swaps the file and keeps the module id, the place in the Section, the name and the visibility. An unchanged document uploads nothing. A retitle in the table is not in the hash, so it is not picked up yet: renaming the resource is left to the Instructor, by hand.
