...
- **A cross-reference can fail to find its target exactly one way.** `LinkToWithheldDocument` is gone: no document is withheld, so a link that does not resolve is a link to a document the table does not name, which is the typo case and still a hard error. _(Superseded by issue #22: a link to a document the table does not name now publishes as its link text alone, and the only link that aborts a run is a Student-facing document linking to Instructor Material.)_ The rule that decided which Resources shipped first — a link to withheld material is a dead link, so the transitive closure of the day-one reading had to ship with it — has no work left to do, because the closure is published either way.
