...
**Grid Frame**: The part of the Assessment Grid the publisher owns, identical in every EPF course: the title, the Band legend, the two gaps, how the Oral verifies a provisional Band, the Feedback Letter, each Competency's heading, and Resit. It is everything a Student reads except the bodies of the Competency blocks. Where it states a course's facts, such as each Freeze, the Oral or a Competency's title, it writes them from `publisher.json` and the Grid Source's front matter, never from prose. A course can neither drop nor reword it; it changes only when the pinned publisher does. _Avoid_: template, boilerplate
