...
**Grid Frame**: The part of the Assessment Grid the publisher owns, identical in every EPF course: the opening line naming the course, programme and term, the Band legend and the two gaps, each Freeze and the Extension sentence, how the Oral verifies a provisional Band, how the Feedback Letter is made, each Competency's heading, and what a Resit means. Three of its parts are printed only where the course declares the facts they state — the Rehearsal, the Reading Day and the Oral's timetable — and a course that declares none reads nothing about them. It prints no title of its own. It is everything a Student reads except the bodies of the Competency blocks. Where it states a course's facts, such as each Freeze, the Oral, the programme or a Competency's title, it writes them from `publisher.json` and the Grid Source's front matter, never from prose. A course can neither drop nor reword it; it changes only when the pinned publisher does. _Avoid_: template, boilerplate

**Region**: A part of the Grid Frame printed only for a course that declares the facts it states — the Rehearsal, the Reading Day and the Oral's timetable — and left out whole, with no heading and no placeholder, for a course that does not (ADR-0015). Its wording is the Frame's, like everything else the Frame says; what the course decides is whether it is printed at all, never how it reads. _Avoid_: optional section, conditional block, toggle
...
**Rehearsal**: A supervised lab, before the Freeze, where a Student puts every Competency into practice on their own work. It counts towards no Band: it is where a Student finds out what they cannot yet do while there is still time. Not every course holds one — a course that does declares when it is and how long it lasts in its Grid Source, and the Grid Frame explains it. _Avoid_: mock, dry run, practice exam

**Reading Day**: The day the Instructor reads every Student's work as it stands, and after which nothing pushed is read. It is stated in the Grid Source and explained by the Grid Frame, never enforced: only a Freeze is enforced, by its Devoir closing. A course whose work is read in several sittings declares none.
...
- **The `Instructors` Section** — Instructor Material is a hidden PDF and sits in the Section its Student counterpart is in (ADR-0004).
