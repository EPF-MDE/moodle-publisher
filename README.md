...
- A document goes in as a **PDF**: the driver prints the rendered HTML with Chromium and uploads it through the resource form's file manager, the only way a file gets into this Moodle. A Devoir's description goes in as HTML source: the driver switches the instructor's editor preference to the plain text area, fills the field, and restores the previous preference when the run ends. Nothing is typed character by character. If the preference cannot be set the run aborts, because filling a rich editor's hidden textarea produces a description that is quietly wrong. The switch happens only when a run is about to write something — a plan never touches the preference, and so can never abort on it. If the restore fails the run still finishes, with a warning naming the value to set back by hand.
- A Devoir update opens **the same form on the existing activity** (`/course/modedit.php?update=<module id>`) and fills the name, the description and the dates and nothing else. Moodle keeps the module id, the section and the visibility, and the driver reads the course back afterwards: an update form a future Moodle stops honouring fails loudly instead of reporting a Devoir rewritten while students carry on reading last week's version.
...
They deliberately favour core Moodle URLs and stable form ids over theme markup: a file resource is created through `/course/modedit.php`, not by clicking through the activity chooser, which is the part a theme is most likely to reskin.
...
The Assessment Grid a Student reads is never written whole: the publisher assembles it at publish time from two parts ([ADR-0014](./docs/adr/0014-the-assessment-grid-is-assembled-from-a-grid-source-and-a-grid-frame.md)), and says nothing about what this course does not have ([ADR-0015](./docs/adr/0015-what-a-course-may-not-have-is-a-region-of-the-grid-frame.md)).
...
**A GitHub reference naming its repository is published as a link.** GitHub links `EPF-MDE/OceENS#97` on its own pages, but it is not markdown, so a course written on GitHub looks linked there and prints as plain text. The publisher links it to `https://github.com/EPF-MDE/OceENS/issues/97`, which GitHub redirects when the number is a pull request's. `#97` alone stays text: GitHub takes its repository from the page it is on, and a PDF is on none. A reference inside a link's text or a code span is left as written. A document making one is folded into the content hash as linking them, so it is republished once rather than kept as the text it was published with.

The title and Section come from the same table entry that decides whether the link may be published at all, never from the filename or from a heading in the target. Both are folded into the document's content hash for the same reason the bytes of a picture are: rename or move an entry in the table and every document linking to it reads differently, which nothing in the linking document's markdown would show.
...
**What it is not.** It is not encryption, and it is not a promise about anyone who is more than a student. A course-level role override, a student who is also a teacher on the course, or anyone with site-level rights can read every word of it. In the 2026 course, the seeded C3 bug's real protection remains the **private GitHub repository** until 8 September; hiding the PDF is what keeps it off the course page, not what makes it secret.

**Why the activity and not a section.** Hiding a section is one click, covers everything in it, and one click the other way — made in a hurry on the day a brief is being revealed — uncovers all of it at once and says nothing afterwards ([ADR-0004](./docs/adr/0004-instructor-material-is-hidden-per-page.md)). So visibility is a property of the activity. Every section the publisher creates is created visible, and no run hides a section.

Four things make the per-activity lock enforceable rather than merely intended:

1. **Created hidden, in one submission.** There is no moment between an instructor PDF being created and being hidden: the resource form carries the visibility, so the PDF is never on the course page showing.
2. **Hidden, never stealthed.** "Available but not shown on the course page" leaves a URL that still works for anyone who has one. The publisher never sets stealth mode, and a driver that finds an activity it just created stealthed **aborts**.
3. **Self-healing, one way only.** A run that finds an instructor PDF visible **re-hides it and says so** in the output, so an accidental reveal is corrected at the next publish rather than persisting until a student finds it. The plan reports the `hide` before anything is applied, and a dry run performs none of it. The driver call it uses takes **no boolean**, so the publisher has no operation that reveals.
...
A lecture that shows a diagram is not published until the diagram is. Nothing in a course repository is a URL Moodle serves, so **every picture a document shows is embedded in the HTML its PDF is printed from**: the PDF carries the drawing, and a printed or offline copy is never missing one. A diagram shown by two documents is inside both PDFs.
...
Nothing in the publisher checks the course after a run. What a human did by hand in Moodle — a revealed PDF of Instructor Material, a Freeze moved, file upload switched back on, a Devoir deleted — is found by reviewing the course by eye, against [docs/human-review-checklist.md](./docs/human-review-checklist.md). The `audit` command that used to read the course for this is retired.
